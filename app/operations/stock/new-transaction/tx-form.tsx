  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/operations/stock" className="text-sm text-slate-500 hover:text-slate-900">← Back to Stock</Link>
      </div>
      ...
      <form id="tx-form" action={onSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select name="stockItemId" className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm">
          ...
          <div>
            ...
            <select name="type" className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm">
              {TX_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Date</label>
            <input type="date" name="transactionDate" required className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="text" name="reference" placeholder="GRN-001..." className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm" />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          {result && <p className={'text-sm ' + (result.success ? 'text-green-600' : 'text-red-600')}>{result.message}</p>}
        </div>
      </form>
    </div>
  );
