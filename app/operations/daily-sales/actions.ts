...
export async function createDailySale(formData: FormData): Promise<SaleActionResult> {
  ...
  if (actual.isNegative()) {
    return { success: false, message: 'Actual sales cannot be negative' };
  }
  if (system.isNegative()) {
    return { success: false, message: 'System sales cannot be negative' };
  }
  ...
  // Check for existing entry (same branch + staff + date)
  ...
  if (existing) {
    return {
      success: false,
      message: 'A sale record already exists for this staff member on this date. Edit the existing record instead.',
    };
  }
