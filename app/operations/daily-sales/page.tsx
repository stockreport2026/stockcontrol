      ...
      <div className="mt-8 bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
        ...
        {recentSales.length === 0 ? (
          <div className="p-8 text-center text-slate-500">
            No sales records yet. Record your first sale above.
          </div>
        ) : (
                ...
                return (
                  <tr key={s.id} className="border-t border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-6 py-3 text-right font-medium text-slate-900">
                      {Number(s.actualSales).toLocaleString()}
                    </td>
                    <td className="px-6 py-3 text-right text-slate-600">
                      {Number(s.systemSales).toLocaleString()}
                    </td>
                    <td className={`px-6 py-3 text-right font-medium ${variance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
