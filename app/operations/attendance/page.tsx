  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">Days worked and leave days per staff, per period</p>
      </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
        ...
        {attendances.length === 0 ? (
          <div className="p-12 text-center text-slate-500">No attendance records yet.</div>
        ) : (
              ...
              {attendances.map((a) => (
                <tr key={a.id} className="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-6 py-3 text-slate-600 text-xs">
                    {new Date(a.periodStartDate).toLocaleDateString('en-GB')} — {new Date(a.periodEndDate).toLocaleDateString('en-GB')}
                  </td>
                  ...
                  <td className="px-6 py-3 text-right"><DeleteAttendanceButton id={a.id} /></td>
                </tr>
              ))}
            </tbody>
          </table>
