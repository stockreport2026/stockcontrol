...
export function BranchForm() {
  ...
  function onSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      ...
      setResult(res);
      if (res.success) {
        const f = document.getElementById('branch-form') as HTMLFormElement;
        f?.reset();
      }
    });
  }
      ...
      <form id="branch-form" action={onSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="name" required placeholder="Kisumu Retail"
              className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Branch Code</label>
            <input type="text" name="code" required placeholder="KSM-001"
              className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Location</label>
            <input type="text" name="location" placeholder="Kisumu"
              className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Selling Days per Period</label>
