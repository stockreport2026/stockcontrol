...
export default async function BranchesPage() {
  const branches = await prisma.branch.findMany({
    orderBy: { name: 'asc' },
    include: { _count: { select: { staff: true, staffSales: true, branchSales: true } } },
  });
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">Add branches to begin entering sales data</p>
      </div>
      <BranchForm />
      <div className="mt-8 bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
        ...
        {branches.length === 0 ? (
          <div className="p-8 text-center text-slate-500">No branches yet. Add your first branch above.</div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="text-right px-6 py-3 font-medium text-slate-600">Staff Sales Records</th>
                <th className="text-right px-6 py-3 font-medium text-slate-600">Branch Sales Records</th>
            ...
            <tbody>
              {branches.map((b) => (
                <tr key={b.id} className="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-6 py-3 text-right text-slate-600">{b._count.staffSales}</td>
                  <td className="px-6 py-3 text-right text-slate-600">{b._count.branchSales}</td>
                  <td className="px-6 py-3 text-right"><DeleteBranchButton id={b.id} name={b.name} /></td>
                </tr>
              ))}
            </tbody>
          </table>
