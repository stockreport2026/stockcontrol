...
export default async function StaffAdminPage() {
  ...
  return (
    <div>
      ...
      <StaffForm branches={branchOptions} />
      ...
      <div className="mt-8 bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
        ...
        {staff.length === 0 ? (
          <div className="p-8 text-center text-slate-500">No staff yet. Add your first staff member above.</div>
        ) : (
            ...
            <tbody>
              {staff.map((s) => (
                <tr key={s.id} className="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-6 py-3 font-mono text-xs text-slate-600">{s.employeeCode}</td>
